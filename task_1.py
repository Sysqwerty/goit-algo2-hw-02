...
def optimize_printing(print_jobs: List[Dict], constraints: Dict) -> Dict:
    ...
    print_order = []
    total_time = 0
    current_volume, current_items, group_time, current_group = 0, 0, 0, []
    ...
    for job in sorted_jobs:
        if (
            ...
            current_items += 1
            group_time = max(group_time, job.print_time)
        else:
            print_order.extend([j.id for j in current_group])
            total_time += group_time
            # скидаємо лічильники за дефолтом
            current_volume, current_items, group_time, current_group = 0, 0, 0, []
            current_group.append(job)
            current_volume = job.volume
            current_items = 1
            group_time = job.print_time
